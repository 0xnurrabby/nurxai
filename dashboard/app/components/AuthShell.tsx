...
export default function AuthShell({ children, mode }: { children: ReactNode; mode: "login" | "signup" | "reset" }) {
  const copy = mode === "signup"
    ? { eyebrow: "HUMAN-IN-THE-LOOP", title: "Your voice. Sharper replies.", body: "NurAi drafts the angles. You edit, ignore, or publish. The final click always stays with you." }
    : mode === "reset"
      ? { eyebrow: "SECURE RECOVERY", title: "Back in. No support ticket.", body: "A short-lived email code verifies ownership, resets the password, and closes every old session." }
      : { eyebrow: "OPERATOR CONSOLE", title: "Move fast. Sound human.", body: "Generate grounded reply options inside X, keep your judgment, and ship only what feels right." };
      ...
      <div className="auth-grid mx-auto grid w-full max-w-5xl overflow-hidden border-2 border-ink dark:border-nightInk lg:grid-cols-[1.08fr_.92fr]">
        <aside className="auth-manifest relative hidden min-h-[620px] overflow-hidden p-10 lg:flex lg:flex-col lg:justify-between">
          <div className="relative z-10">
            ...
            <h2 className="mt-8 max-w-lg font-display text-6xl font-black leading-[.9] tracking-[-.05em]">{copy.title}</h2>
